...
//     if (!player.isThief) {
//       io.to(player.).emit("wordAssigned", room.currentWord);
//     }
//   });
...
const roomTimers: Record<
  string,
  { seconds: number; interval?: NodeJS.Timeout; running: boolean }
> = {};
...
io.on("connection", (socket) => {
  ...
  socket.on("startGame", ({ roomCode, category, word }) => {
    ...
    const wordsForCategory = WORDS[category];
    if (!wordsForCategory || wordsForCategory.length === 0) return;

    room.status = "playing";
    room.category = category;
    room.currentWord =
      word && wordsForCategory.includes(word)
        ? word
        : randomItem(wordsForCategory);

    const thiefIndex = Math.floor(Math.random() * room.players.length);
    room.players.forEach((p, i) => (p.isThief = i === thiefIndex));
    room.thiefId = room.players[thiefIndex].id;

    // Send word only to non-thief players
    room.players.forEach((player) => {
      if (!player.isThief) {
  ...
  /* ---------- SET TIMER ---------- */
  socket.on("setTimer", ({ roomCode, timer }) => {
    const room = rooms[roomCode];
    if (!room) return;
    // Clear previous timer
    if (roomTimers[roomCode]?.interval)
      clearInterval(roomTimers[roomCode].interval);
    roomTimers[roomCode] = { seconds: timer, running: true };
    io.to(roomCode).emit("timerUpdate", {
      timer: roomTimers[roomCode].seconds,
      running: true,
    });
    roomTimers[roomCode].interval = setInterval(() => {
      if (!roomTimers[roomCode]) return;

      roomTimers[roomCode].seconds -= 1;

      io.to(roomCode).emit("timerUpdate", {
        timer: roomTimers[roomCode].seconds,
        running: roomTimers[roomCode].seconds > 0,
      });
      if (roomTimers[roomCode].seconds <= 0) {
        clearInterval(roomTimers[roomCode].interval);
        roomTimers[roomCode].running = false;
      }
    }, 1000);
  });
...
function calculateResults(room: Room) {
  ...
  room.players.forEach((player) => {
    if (votes[player.id] === maxVotes) player.score += 1;
    delete player.vote;
  });
}
